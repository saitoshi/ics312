...
Q3_SHIFT:
  ...
  jmp question4_initial ; jmp to question4_initial

question4_initial:
	mov eax, msg5 ; point eax to msg5
	call print_string ; print the msg5 string
	mov ebx, [input] ; mov ebx to store input
	mov ecx, 008h ; mov ecx to 008h to act as an index marker
	mov edx, 000h ; mov edx to 000h to act as a counter
	jmp question4

question4:
	mov eax, 005h ; point eax to 005h
	and eax, ebx ; take the intersection of eax, ebx
	cmp eax, 001h ; compare eax with 001h (0001b)
	jne q4_shift ; if not equal to jump to q4_shift
	inc edx ; increment edx otherwise

q4_shift:
	shr ebx, 4 ; shift ebx to the 4 to the right
	loop question4 ; loop though question4
	mov eax, edx ; point eax to edx
	call print_int ; print the integer
	call print_nl ; print new line
	jmp close_program ; jump to close program
